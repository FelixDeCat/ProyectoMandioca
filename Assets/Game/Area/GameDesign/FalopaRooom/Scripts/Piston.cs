...
public class Piston : MonoBehaviour
{
    ...
    public virtual void Start()
    {
        ...
        pingponglerp.Configure(AnimationResult, true, true, stay_position_time, gameObject.transform.parent.gameObject.name);
    ...
    {
        if (ac_hitFloor) AudioManager.instance.PlaySound(ac_hitFloor.name, transform);
    }
    public void AnimationResult(float val_anim)
    {
        ToMove.position = Vector3.Lerp(_startPos.position, _EndPos.position, val_anim);
    }
