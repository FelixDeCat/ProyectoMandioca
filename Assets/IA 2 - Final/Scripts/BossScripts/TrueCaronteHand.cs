...
public class TrueCaronteHand : MonoBehaviour
{
    ...
    {
        if (1 < other.gameObject.layer & maskWall != 0) { WallCollision.Invoke(); return; }

        if (other.gameObject.GetComponent<DamageReceiver>()) other.gameObject.GetComponent<DamageReceiver>().TakeDamage(dmgData.SetDamage(damage).SetDamageInfo(dmgInfo)
